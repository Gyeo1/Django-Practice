...
class BaseModel(models.Model):  # 굳이 Basemodel이라고 할 필요 x
    created_at = models.DateTimeField(
        auto_now_add=True)  # auto_now_add로 바로 작성되도록
...
class Post(BaseModel):
    ...
    def extract_tag_list(self):  # 이 함수는 tag의 모든 값을 뽑아오기 위해 사용한다.
        tag_name_list = re.findall(
            r"#([a-zA-Z\dㄱ-힇]+)", self.caption)  # 알파벳과 한글을 뽑아낸다
        tag_list = []
        for tag_name in tag_name_list:  # tag_name_list의 리스트 값을 하나씩 가져옴
            tag, _ = Tag.objects.get_or_create(
                name=tag_name)  # 리스트 값을 tag란 이름의 값으로 만든다?
